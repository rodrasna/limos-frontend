...
import { getDiet } from "../graphql/queries";
import { updateDiet } from "../graphql/mutations";
export default function DietUpdateForm(props) {
  const {
    id: idProp,
    diet: dietModelProp,
    onSuccess,
  ...
  const resetStateValues = () => {
    const cleanValues = dietRecord
      ? { ...initialValues, ...dietRecord }
      : initialValues;
    setClientID(cleanValues.clientID);
    setClient(cleanValues.client);
    setDateDelivery(cleanValues.dateDelivery);
    setDateConsult(cleanValues.dateConsult);
    setTimeRealization(cleanValues.timeRealization);
    setCalories(cleanValues.calories);
    setErrors({});
  };
  const [dietRecord, setDietRecord] = React.useState(dietModelProp);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? (
            await API.graphql({
              query: getDiet.replaceAll("__typename", ""),
              variables: { id: idProp },
            })
          )?.data?.getDiet
        : dietModelProp;
      setDietRecord(record);
    };
    queryData();
  }, [idProp, dietModelProp]);
  React.useEffect(resetStateValues, [dietRecord]);
  const validations = {
    ...
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        let modelFields = {
          clientID,
          client: client ?? null,
          dateDelivery: dateDelivery ?? null,
          dateConsult: dateConsult ?? null,
          timeRealization: timeRealization ?? null,
        ...
        try {
          ...
          await API.graphql({
            query: updateDiet.replaceAll("__typename", ""),
            variables: {
              input: {
                id: dietRecord.id,
                ...modelFields,
      ...
      {...getOverrideProps(overrides, "DietUpdateForm")}
      {...rest}
    >
      <TextField
        label="Client id"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || dietModelProp)}
